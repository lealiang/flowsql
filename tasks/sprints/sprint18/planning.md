...
- [ ] SQL 工作台中所有任务均可进入“可视化”页面（不按 task kind 限制）。
- [ ] 可视化页面以单画布展示完整 DAG，包含 `Channel` 与 `Operator` 两类节点。
- [ ] 同名通道节点去重，能将多 SQL 自动串接为同一张图。
- [ ] 边能区分触发语义：`on_start`、`on_data`、`on_finish`。
- [ ] 节点/边展示运行状态与关键指标（`status/phase/processed_rows/output_rows/error`）。
- [ ] 支持轮询刷新，终态自动停止轮询。
- [ ] 后端返回统一图结构 JSON，前端不依赖 SQL 字符串解析做主逻辑。
- [ ] 新增 URI 的 Method/路径/错误码映射已冻结并在设计与计划文档同步。
- [ ] 自动化测试通过（后端单测/集成 + 前端构建）。
...
|---|---|---|---|---:|---|---|
| T1 | 待开始 | 冻结 Runtime Graph URI 与 JSON 契约（Method/路径/错误码/字段） | `sprint18/design.md`、`scheduler_*`、`task_plugin.cpp` | 0.8 | - | 契约冻结，字段与 URI 语义文档化 |
| T2 | 待开始 | Scheduler 侧构建 DAG 快照（节点去重、边触发语义、依赖映射） | `scheduler_stream_group_*` | 1.6 | T1 | 可按 runtime_task_id 输出完整图结构 |
| T3 | 待开始 | TaskPlugin 转译并对外暴露任务图查询能力 | `task_plugin.cpp`、`web_server.cpp` | 1.0 | T2 | 通过 task_id 查询图快照 |
| T4 | 待开始 | 前端 Runtime 页改为图驱动渲染（非 SQL 行渲染） | `TaskRuntime.vue` | 1.8 | T1/T3 | 单画布 DAG 展示完成 |
| T5 | 待开始 | 边触发语义样式与节点状态联动（on_start/on_data/on_finish） | `TaskRuntime.vue`、样式文件 | 1.0 | T4 | 触发语义可视化可区分 |
| T6 | 待开始 | 增量刷新与终态收敛（cursor 轮询 + `snapshot_time_ms` 展示） | `TaskRuntime.vue`、`api/index.js` | 0.8 | T4 | 轮询稳定，无重复抖动 |
| T7 | 待开始 | SQL 工作台可视化入口与跳转链路收口 | `Tasks.vue`、`router/index.js` | 0.5 | T4 | 所有任务可进入可视化页 |
| T8 | 待开始 | 后端测试补齐（图结构正确性、触发语义、多 SQL 串接） | `test_scheduler_e2e.cpp`、`test_task.cpp` | 1.0 | T2/T3 | 关键场景自动化覆盖 |
| T9 | 待开始 | 前端回归与文档同步（README/迭代文档） | `README.md`、`planning.md` | 0.5 | T5/T6/T7 | 能力描述与实现一致 |
