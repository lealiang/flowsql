# AGENTS.md


## 工作流编排

### 1. 默认规划节点 (Plan Node Default)
- 对于任何非平凡任务（3个以上步骤或涉及架构决策），**必须**进入规划模式。
- 如果事情偏离轨道，**立即停止**并重新规划——不要强行推进。
- 将规划模式用于验证步骤，而不仅仅是构建步骤。
- 预先编写详细的规格说明以减少歧义。

### 2. 子代理策略 (Subagent Strategy)
- 大量使用子代理以保持主上下文窗口整洁。
- 将研究、探索和并行分析工作卸载给子代理。
- 对于复杂问题，通过子代理投入更多算力。
- 每个子代理专注于一个具体的切入点以确保执行聚焦。

### 3. 自我改进循环 (Self-Improvement Loop)
- 收到用户纠正后，先判断它是否暴露了跨任务、跨模块可复用的原则或方法论。
- 只有具备普适性的模式才进入 `tasks/lessons.md`；一次性事实、具体路径、端口、临时实现细节不生成全局规则。
- 新增规则前先查是否能合并到已有条目，优先改写和压缩旧规则，避免按纠正次数堆叠条目。
- 在会话开始时按当前任务定向审查相关教训，不为了“了解背景”全文扩散。

### 4. 完成前验证 (Verification Before Done)
- 在没有证明其有效之前，**绝不**标记任务为完成。
- 在相关情况下，对比主分支与你更改后的行为差异。
- 自问：“资深工程师会批准这个吗？”
- 运行测试、检查日志、展示正确性。

### 5. 追求优雅（平衡之道）(Demand Elegance - Balanced)
- 对于非平凡的变更：暂停并自问“是否有更优雅的方式？”
- 如果修复感觉像是在打补丁（hacky）： “鉴于我现在所知的一切，实施那个优雅的解决方案。”
- 对于简单、明显的修复可跳过此步——不要过度工程化。
- 在提交成果前先挑战自己的工作。

### 6. 自主缺陷修复 (Autonomous Bug Fixing)
- 当收到缺陷报告时：**直接修复它**。不要请求手把手的指导。
- 指向日志、错误或失败的测试——然后解决它们。
- 无需用户进行任何上下文切换。
- 主动修复失败的 CI 测试，无需被告知如何做。

## 任务管理（Scrum 框架）

### 文档结构
```
tasks/
├── product_backlog.md          # 产品待办列表（Epic + Story）
├── sprints/
│   └── sprintN/
│       ├── planning.md         # 迭代规划
│       ├── design.md           # 迭代设计
│       ├── review.md           # 迭代评审
│       └── retrospective.md    # 迭代回顾
└── lessons.md                  # 经验教训汇总
```

### Sprint Planning（迭代规划）
1. **选取 Story**：从 `product_backlog.md` 选取优先级高的 Story
2. **任务分解**：将 Story 分解为具体的 Task，写入 `sprintN/planning.md`
3. **估算工作量**：评估每个 Story 的工作量（天数）
4. **明确验收标准**：每个 Story 必须有清晰的验收标准
5. **识别风险**：列出技术风险和缓解措施
6. **记录设计决策**：重要的架构决策要记录理由和影响

### Sprint Design（迭代设计）
1. **任务化编排**：`design.md` 采用“基础设计与公共依赖 + 按 planning Task 编排”的结构，不按孤立技术主题平铺。
2. **公共设计准入**：被 2 个以上 Task 依赖，或会冻结跨 Task 的 public ABI、身份、生命周期、并发、错误语义和测试基础设施时，才进入公共设计；公共设计必须标明 owner Task 和 consumer Task。
3. **设计责任判定**：Task 只要包含需要在实施前冻结的实现决策、测试方法或验收门槛，就必须在 `design.md` 中有对应章节；测试、性能、Sanitizer、TSan、ABI 验证同样属于设计。
4. **流程 Task 例外**：纯状态更新、证据汇总或行政性工作可以不设独立设计章节，但必须在 `planning.md` 中注明“无独立设计责任”；若其定义证据格式、验证命令或通过门槛，则仍需设计章节。
5. **章节固定模板**：每个 Task 章节至少包含设计目标、验收 ID、公共依赖、实现/验证方案、文件落点、测试设计与通过门槛、完成出口；不适用项必须明确标注。
6. **双向同步**：`planning.md` 是 Task、依赖、估算和状态的唯一来源，`design.md` 是实现方法、测试方法和技术决策的唯一来源；Task 增删、拆分或重编号时必须在同一次变更中同步两份文档。
7. **结构即映射**：Task 章节标题直接使用 planning Task ID 和名称，不再依赖文末“技术章节到 Task”的补偿性反向映射表。

### Sprint 执行
1. **追踪进度**：在 `planning.md` 中标记任务完成状态（[x]）
2. **同步更新**：每完成一个 Story，立即更新文档
3. **代码审查**：每个 Story 完成后立即进行代码审查
4. **测试验证**：运行测试、检查日志、展示正确性
5. **文档同步**：代码和文档必须同步更新

### Sprint Review（迭代评审）
1. **验收 Story**：逐个验收 Story，确认是否达成验收标准
2. **功能演示**：演示已完成的功能
3. **识别技术债务**：记录发现的技术债务和改进计划
4. **更新产品待办**：将已完成的 Story 标记为 ✅，新增 Story 标记为 📋
5. **规划下一个 Sprint**：确定下一个 Sprint 的目标

### Sprint Retrospective（迭代回顾）
1. **做得好的地方（Keep）**：总结成功经验，继续保持
2. **需要改进的地方（Problem）**：识别问题、分析根因、制定改进措施
3. **尝试的新做法（Try）**：评估新做法的效果，决定是否继续
4. **行动项（Action Items）**：明确负责人、截止日期、状态跟踪
5. **经验教训固化**：将关键教训提炼到 `lessons.md` 和 `AGENTS.md`

### Story 管理
- **Epic**：大的功能模块（如"数据库闭环能力"）
- **Story**：可独立交付的功能点（如"SQLite 驱动实现"）
- **Task**：Story 的具体实施步骤（如"实现 SqliteBatchReader"）
- **状态标识**：✅ 已完成、🚧 进行中、📋 待规划、💡 设计阶段

### 经验教训管理
1. **准入筛选**：只有跨任务、跨模块、跨迭代可复用的原则和方法论，才写入 `tasks/lessons.md`
2. **禁止膨胀**：用户每次纠正不自动生成新规则；单次失误、环境操作、具体文件路径、端口和阶段实现细节不进入 lessons
3. **合并优先**：新增内容前先查已有条目，能补充或改写旧条目就不新增编号
4. **分流归属**：项目操作规范放 `AGENTS.md` 或 README，Sprint 细节放对应 sprint 文档，接口和配置约束放设计或配置文档
5. **定期瘦身**：发现重复、过细或失去普适性的条目时，优先合并、迁移或删除

### 文档更新原则
1. **代码与文档同步**：每个 Story 完成后立即更新文档
2. **文档要准确**：确保文档与实际代码一致
3. **文档要完整**：用户手册、开发者指南、API 文档都要完善
4. **测试也是设计**：测试样本、失败条件、性能基线、并发验证和 Sanitizer 入口必须在对应 Task 设计中提前冻结，不能只在 planning 中列命令。

## 架构原则

1. **统一框架（Python Worker 例外）**：C++ 服务都是同一个框架程序加载不同 .so 运行起来的，彼此对等；Python Worker 是独立的 FastAPI 进程
2. **IPlugin 机制保留**：所有功能模块以 .so 插件形式加载
3. **控制面 HTTP 统一**：服务间控制面通信全部走 HTTP + URI 路由
4. **数据面独立通道**：高吞吐数据传输走共享内存 / Arrow IPC，不经过 HTTP
5. **Interface 思维**：Plugin 通过 interface（纯虚基类）向本进程内的其他 Plugin 暴露能力，调用方通过 IQuerier 按 IID 查找，不直接依赖具体实现类
6. **IPlugin 生命周期批次调用**：插件加载必须分阶段批次执行——先所有插件 Option()，再所有插件 Load()，最后所有插件 Start()。禁止逐个插件走完整个生命周期（Option→Load→Start 再下一个），因为 Start() 时必须保证所有插件的接口已通过 Load() 注册完毕

## 核心原则

- **简洁优先**：使每次变更尽可能简单。影响最小化的代码。
- **拒绝懒惰**：寻找根本原因。杜绝临时修复。遵循资深开发者标准。
- **最小影响**：变更应仅触及必要部分。避免引入新的 bug。

## 交流规则

1. 全程使用中文交流
2. 需要编译时直接编译，无需确认
3. 移动或删除目录前需要用户确认
4. 不要自动提交代码（git commit/push），等用户明确发起提交指令后再执行

## 编程规则

- 尽量使用成熟的开源方案，不重复造轮子
- 给代码提供必要的中文注释
- `git worktree` 开发统一复用主工作区的 `.thirdparts_installed` 和 `.thirdparts_prefix`，禁止每个 worktree 各自维护一套第三方编译缓存
- 新增 `thirdparts/*-config.cmake` 或新增第三方依赖时，必须先在主工作区完成一次下载 / 编译 / 安装缓存，再进入 `.worktrees/` 分支开发
- 头文件库也遵循同一规则；不要因为“只包含头文件”就跳过主工作区缓存预热，例如 `Eigen 3`
- 本项目仓库根目录固定为 `flowSQL`（大小写敏感），默认构建入口固定为 `src/CMakeLists.txt`；凡是执行项目编译，默认使用 `cmake -B build src` 与 `cmake --build build -j$(nproc)`，禁止再次通过搜索 `CMakeLists.txt` 来猜测入口
- 新建或修改 `*.h/*.hpp/*.cpp/*.cc` 文件时，文件头必须使用统一版权声明：
  `Copyright (C) 2026 LIHUO` + MIT License 说明块
- 问题处理必须先核查再修改（强制）：
  1. 收到问题后先做事实核查（日志/代码路径/接口返回/测试复现），禁止直接改代码
  2. 核查确认是问题：先给出最小修改方案（影响面、风险、验证方式），再实施修改
  3. 核查确认不是问题：必须提供可复核证据（命令输出、代码位置、接口响应）并说明不改原因
  4. 任何“未核查先修改”的行为都视为流程违规

## 代码风格

- C++17，命名空间 `flowsql`（子空间如 `flowsql::protocol`）
- clang-format 配置在 `src/.clang-format`：Google 风格基础上 4 空格缩进、120 列宽
- `interface` 宏定义为 `struct`（见 `common/define.h`），用于声明纯虚接口
