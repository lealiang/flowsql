...
**不做的事（已移至 [Stage 3](stage3.md)）：**
- 异步任务执行
- SQL 列选择（`SELECT col1, col2`）
- C++ 算子 / 网卡通道的实际实现
- DatabaseChannel 具体实现（ClickHouse 等）
- 存储算子 / 提取算子的实际实现
- 多算子 Pipeline / 并行执行
- 用户认证
...
以下任务已移至 [Stage 3 设计文档](stage3.md)：
- P1：DatabaseChannel 实现 + 存储/提取算子（数据库闭环）
- P2：SQL 列选择 + 多算子 Pipeline
- P3：异步任务执行 + Gateway 统一认证
- 流式架构（IStreamChannel / DPDK / StreamWorker）
...
### 步骤 8：集成测试 — ✅ 已完成（通过端到端手动验证）
- 端到端验证：启动 flowsql → 上传算子 → 提交 SQL → 查看结果
